...
if (!empty($gallery_data)) {
	...
	$position = $gallery_data->position;
	$file = $gallery_data->file;
	$file_title = $gallery_data->file_title;
			...
			<div class="col-12">
				...
				<div class="card">
					...
					if ($user_access->view_module == 1 || true) {
						...
						<div class="card-body">
							<?php echo form_open(
								MAINSITE_Admin . "$user_access->class_name/galleryDoEdit",
								array(
									'method' => 'post',
									'id' => 'gallery_form',
									"name" => "gallery_form",
							...
							<div class="form-group row">
								...
								<div class="col-lg-3 col-md-4 col-sm-6">
									<label for="file_title" class="col-sm-12 label_content px-2 py-0">Image Title<span
											style="color:#f00;font-size: 22px;margin-top: 3px;">*</span></label>
									<div class="col-sm-12">
										<input type="text" class="form-control form-control-sm" required id="file_title" name="file_title"
											value="<?= $file_title ?>" placeholder="Image Title">
									</div>
								</div>
								<div class="col-lg-3 col-md-4 col-sm-6">
									<label for="position" class="col-sm-12 label_content px-2 py-0">Image Position No.<span
											style="color:#f00;font-size: 22px;margin-top: 3px;">*</span></label>
									<div class="col-sm-12">
										<input type="number" min="1" class="form-control form-control-sm" required id="position"
											name="position" value="<?= $position ?>" placeholder="Image Position No.">
									</div>
								</div>
								<div class="col-md-3 col-sm-6">
									<label for="location" class="col-sm-12 label_content px-2 py-0">Upload Image <span
											style="color:#f00;font-size: 22px;margin-top: 3px;">*</span></label>

									<div class="col-sm-12 d-flex">
										<div class="input-group" style="width:90%">
											<div class="custom-file">
												<input type="file" name="file" class="custom-file-input" id="file" <? if (empty($gallery_data->file)) { ?> required <? } ?>>
										...
										<div class="custom-file-display custom-file-display0">
											<? if (!empty($file)) { ?>
												<span class="pip pip0">
													<a target="_blank" href="<?= _uploaded_files_ . 'main_gallery/' . $gallery_data->file ?>">
...
<script>
	...
	function del_tdate($gallery_date_id) {
		if (parseInt($gallery_date_id) > 0) {
			var s = confirm('You want to delete this file?');
			if (s) {
				$.ajax({
					...
					//dataType: "json",
					data: {
						"table_name": "gallery_date",
						"id_column": "gallery_date_id",
						'id': $gallery_date_id,
						"<?= $csrf['name'] ?>": "<?= $csrf['hash'] ?>"
					},
					success: function (response) {
	...
	function del_plan($gallery_plan_id) {
		if (parseInt($gallery_plan_id) > 0) {
